...
Release 同时提供 **CLI 裸二进制**（`try_*`）与 **`fyne package` 官方 GUI 包**（`try-gui_*`）。安装脚本是适配器，也可只下载官方包手动安装。详情见 [`spec/distribution.md`](spec/distribution.md)。

### 一键安装（推荐）

**Linux / macOS** — 安装 TUI 与 GUI（有官方包时），并执行 `try install`：
...
| 平台 | Release 文件 | 步骤 |
|------|--------------|------|
| macOS | `try-gui_darwin_<arch>.app.zip` | 解压后将 `Try.app` 拖入「应用程序」；若拦截则右键 → 打开 |
| Windows | `try-gui_windows_amd64.zip` | 解压运行 `try-gui.exe`；或用 `install.ps1` 写入开始菜单 |
| Linux amd64 | `try-gui_linux_amd64.tar.gz` | `sudo tar -xzf … -C /` 装到 `/usr/local`，或用 `install.sh` 适配到 `~/.local` |
| Linux arm64 | （无 GUI 官方包） | 使用 TUI，或源码构建 `try-gui` |
...
`try install` 负责 Shell 包装函数与默认配置文件（**不**创建桌面入口）。一键脚本会自动调用；仅手动装 GUI 时，若还要用终端 `try`，请另跑 `try install`。支持 Bash、Zsh、Fish。

### 支持的平台

| OS | 架构 | GUI 官方包 |
|----|------|------------|
| Linux | amd64, arm64 | amd64 有；arm64 仅 CLI |
| macOS | amd64 (Intel), arm64 (Apple Silicon) | 有（`.app`） |
| Windows | amd64 | 有 |
...
`try-gui` 是与 TUI 并列的跨平台原生桌面入口，读取同一份配置，复用相同的目录扫描、模糊匹配与文件操作逻辑。启动后显示原生应用窗口，默认尺寸约 900 × 600，最小尺寸约 720 × 480，内容区配色与快捷键与 TUI 对齐。应用显示名为 **Try**，App ID 为 `com.loveloki.try.gui`。

```bash
try-gui                # 启动 GUI（或从应用列表 / 开始菜单打开 Try）
...
# Windows（需 MinGW；Release 使用 -H=windowsgui）
CGO_ENABLED=1 go build -ldflags "-H=windowsgui" -o dist/try-gui.exe ./cmd/try-gui

# 产出官方包（需 fyne CLI）：./scripts/package-gui.sh <goos> <goarch> <version>
