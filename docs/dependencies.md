...
| `charm.land/lipgloss/v2` | v2.0.3 | 声明式终端样式。用于所有 TUI 渲染：文字颜色/背景色/粗体、ANSI-aware 宽度计算（`Width()`）、文本截断（`Truncate()`/`TruncateLeft()`）、水平对齐（`PlaceHorizontal()`）。NO_COLOR 支持通过 color profile 实现。 |
...
| `github.com/sahilm/fuzzy` | bubbles 间接引入，`internal/fuzzy` 直接 import | 子序列匹配 + 字符位置追踪（仅使用匹配功能，评分和排序由 `internal/fuzzy` 自定义实现）。虽由 bubbles 传递引入，但因为代码中直接 import，go.mod 中会被标记为 direct require。 |
| `github.com/charmbracelet/harmonica` | bubbles | 动画缓动函数（spinner 等组件使用） |
...
> Go 最低版本：1.24（Charm v2 系列要求）。实际 `go.sum` 和间接依赖由 `go mod tidy` 生成。
