...
- 兼容旧配置：若 `ships` 未设置但 `ship`（字符串）有值，自动转换为单元素数组
...
2. ~/.config/try/config.json 中的 ships（优先）或 ship（兼容旧配置）
...
默认值：`Path="~/src/tries"`, `Ships=["~/src/ship", "~/src/bug"]`, `Theme="auto"`, `Locale="auto"`。
