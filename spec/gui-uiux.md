...
- **键盘驱动**：Enter、Ctrl-T、Ctrl-D、Ctrl-R、Ctrl-G、Ctrl-K、Esc、Tab、Ctrl-P/N 等与 TUI 行为对齐。
...
| selector | Ctrl-K | 对话框 → files | 克隆 Git 仓库，成功后进入文件视图 |
...
5. 底栏：左侧项目计数（DeleteMode 下为删除模式文案）；右侧快捷键键帽（↑↓ / Enter / ⌃T / ⌃D / , 设置）。
...
5. 底栏：左侧项目计数；右侧快捷键键帽（↑↓ 导航、Esc 返回、「拖拽上传」提示、, 设置）。

工具栏「编辑」打开选中项（目录进入、文件用系统默认程序）。

「打包 .docx」：选中或唯一勾选须为目录；用 ZIP 打成同级 `.docx`（zip 根为目录内容，不含目录名自身）。目标已存在则写为 `原名-<时间戳>.docx`。

「解压 .docx」：选中或唯一勾选须为 `.docx` 文件；解压到同级目录。目录已存在则解压到 `原名-<时间戳>/`。ZIP 条目做路径穿越防护。
...
- 默认列表视图，隐藏 `.` 开头的条目。
- 进入文件视图或切换 `FilesPath` 时，`fileSelected = -1`（无行高亮）；列表调用 `UnselectAll`。
- 行首复选框：勾选/取消勾选将该项加入多选集合（`fileMarked`）；删除等批量操作优先使用已勾选项。
- Ctrl+点击（macOS 亦认 ⌘）：切换该项勾选，不清除其余已选项。
- 普通单击：移动光标（`fileSelected`），不改变勾选集合。
- 右键菜单（Secondary Tap）：弹出 `widget.NewPopUpMenu`，包含以下项：
  - 「打开」— 等同于双击（默认应用打开）。
  - 「用...打开」— 子菜单，列出内置常见应用（VS Code、Vim 等，按平台过滤可用性）。用户可通过 `config.openWith` 配置扩展名→应用名映射：精确扩展名命中时只显示映射应用（覆盖）；无精确命中而存在通配 `*` 时，通配应用置顶并与内置列表并存显示（含无扩展名文件与目录）。映射命中内置应用时只保留该项；未命中时按自定义应用解析（macOS 应用名自动补 `.app`、PATH 中的可执行名、绝对路径）；精确映射解析失败则子菜单为空，通配应用解析失败回退内置列表。
  - 「在文件夹中显示」— 调用系统文件管理器揭示文件所在目录。
  - 「重命名」— 行内输入框修改文件名。
  - 「删除」— 模态确认对话框。
- 进入子目录：Enter / 双击文件夹。
- 打开文件：Enter / 双击；无选中时 Toast 提示无选中项。
- 导航：无选中时第一次 ↓ 落在首项、↑ 落在末项；已有选中时 ↑/↓ 与 Ctrl-P/N 在首尾循环。
- 目录监听：进入 Files 后对当前 `FilesPath` 使用 `fsnotify` 监听（非递归）；`Create`/`Write`/`Remove`/`Rename` 经约 250ms 防抖后刷新列表；忽略纯 `Chmod`。刷新时按路径尽量恢复光标，路径消失则回到无选中。拖入复制期间暂停监听。离开 Files 或关闭窗口时停止监听。
...
全屏视图（`view = "settings"`），非模态对话框——所有修改即时生效并写回 `config.json`。入口：选择器与文件视图内容区头部的齿轮按钮，或快捷键 Cmd+,（macOS）/ Ctrl+,（Windows/Linux）；设置页内再次按下快捷键或 Esc、点击头部「返回」按钮关闭，返回进入前的视图（selector / files）。语言或主题切换后设置页原地重建并停留在设置页。

布局：头部（标题 + 返回按钮）、内容区（纵向滚动，最大宽度 680px 居中）、底栏（Esc 返回提示）。内容分为两组，每组为小标题 + 圆角面板：

- **外观**：主题（Dark / Light / Auto）与语言（English / 中文 / Auto）两个三选一下拉（固定宽度）。主题为 Auto 时走 `config.DetectTheme()` 的 `COLORFGBG` 检测；切换后立即应用并写回 `config.json` 的 `theme` 字段。语言切换后调用 `i18n.Init` 并重建视图，文案立即以新语言呈现，并写回 `locale` 字段。
- **打开方式**：`openWith` 扩展名→应用名映射的编辑。列表展示现有映射（行间分隔线，每条可单独删除）；添加行由固定宽度扩展名输入框、可搜索应用选择器（占满剩余宽度）与高亮「添加」按钮组成。扩展名输入自动转为小写并补 `.` 前缀（如输入 `go` 自动变为 `.go`），最终须为 `.` 后跟字母或数字；特殊值 `*` 表示通用映射（所有文件，与内置列表并存）；应用名 trim 后非空且不得包含 `..` 路径遍历片段，不合法时均显示校验提示。修改写回 `config.json` 的 `openWith` 字段，成功后 Toast 反馈已添加的映射。

#### 6.5.1 可搜索应用选择器（CompletionEntry）

使用 fyne-x（`fyne.io/x/fyne/widget`）的 `CompletionEntry`——Fyne 社区的自动补全输入框，解决原生 `SelectEntry` 下拉不支持过滤、`PopUpMenu` 过长不可滚动的问题：

- 候选为内置应用 + 已安装应用（去重）：macOS 扫描 `/Applications`、`~/Applications`、`/System/Applications`（含各自 `Utilities` 子目录）的 `.app` 应用名；Linux/Windows 为 PATH 中的可执行名（全量，不截断）。也允许手动输入任意应用名。
- `CompletionEntry` 不内置弹层触发：使用方在 `OnChanged` 中过滤候选并调用 `SetOptions` + `ShowCompletion`；输入为空或无匹配时 `HideCompletion`。选中菜单项时组件内部暂停重开，避免弹层循环。
- 输入即过滤候选（子串匹配）并弹出列表；弹层宽度对齐输入框，高度限制在窗口可用空间内，超出可滚动。
- 弹层内 ↑↓ 导航（循环）、Enter 选中并填入输入框、Esc 收起，点击行直接选中；选中后焦点归还输入框。
...
| Delete | 删除标记项或打开删除确认 |
| Cmd+,（macOS）/ Ctrl+,（Windows/Linux） | 打开设置页；设置页内按下则关闭 |
...
Esc 在设置页：应用选择器弹层展开时先收起弹层，否则关闭设置页并返回进入前的视图。设置页不响应导航与操作键（↑↓ / Enter / Space / Delete / Tab 等），控件按键由各自组件处理。
