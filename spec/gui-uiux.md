...
- 右键菜单（Secondary Tap）：弹出 `widget.NewPopUpMenu`，包含以下项：
  - 「打开」— 等同于双击（默认应用打开）。
  - 「用...打开」— 子菜单，列出内置常见应用（VS Code、Vim 等，按平台过滤可用性）。用户可通过 `config.openWith` 配置扩展名→应用名映射覆盖，优先级：精确扩展名 > 通配 `*` > 内置列表。映射命中内置应用时只显示该应用；未命中时按自定义应用解析（macOS 应用名自动补 `.app`、PATH 中的可执行名、绝对路径），解析失败则子菜单为空。通配 `*` 为通用映射，在无精确命中时生效，含无扩展名文件与目录。
...
- **打开方式**：`openWith` 扩展名→应用名映射的编辑。列表展示现有映射（行间分隔线，每条可单独删除）；添加行由固定宽度扩展名输入框、可搜索应用选择器（占满剩余宽度）与高亮「添加」按钮组成。扩展名输入自动转为小写并补 `.` 前缀（如输入 `go` 自动变为 `.go`），最终须为 `.` 后跟字母或数字；特殊值 `*` 表示通用映射（所有文件）；应用名 trim 后非空且不得包含 `..` 路径遍历片段，不合法时均显示校验提示。修改写回 `config.json` 的 `openWith` 字段，成功后 Toast 反馈已添加的映射。
