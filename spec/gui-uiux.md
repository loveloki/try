...
- 右键菜单（Secondary Tap）：弹出 `widget.NewPopUpMenu`，包含以下项：
  - 「打开」— 等同于双击（默认应用打开）。
  - 「用...打开」— 子菜单，列出内置常见应用（VS Code、Vim 等，按平台过滤可用性）。用户可通过 `config.openWith` 配置扩展名→应用名映射覆盖。
...
模态对话框（`dialog.NewCustom`），仅「关闭」按钮，无保存/取消——所有修改即时生效并写回 `config.json`。入口：选择器与文件视图内容区头部的齿轮按钮，或快捷键 Cmd+,（macOS）/ Ctrl+,（Windows/Linux）。

对话框内容分为两组：

- **外观**：主题（Dark / Light / Auto）与语言（English / 中文 / Auto）两个三选一下拉。主题为 Auto 时走 `config.DetectTheme()` 的 `COLORFGBG` 检测；切换后立即应用并写回 `config.json` 的 `theme` 字段。语言切换后调用 `i18n.Init` 并重建当前视图，文案立即以新语言呈现，并写回 `locale` 字段。
- **打开方式**：`openWith` 扩展名→应用名映射的编辑。列表展示现有映射；添加行由扩展名输入框与内置应用 `SelectEntry` 组成；每条映射可单独删除。扩展名输入自动转为小写，须为 `.` 后跟字母或数字（如 `.go`、`.7z`），不合法时显示校验提示。修改写回 `config.json` 的 `openWith` 字段。
...
| Delete | 删除标记项或打开删除确认 |
| Cmd+,（macOS）/ Ctrl+,（Windows/Linux） | 打开设置对话框 |
